...
class TizenDriver extends BaseDriver {
  constructor (opts = {}, shouldValidateCaps = true) {
    ...
    log.debug(`TizenDriver version: ${version}`);
  ...
  async startTizenSession () {
    if (this.opts.reboot) {
      await this.sdb.root();
      await this.sdb.reboot();
      await sleep(40000);

      await this.sdb.stopAutoSleep();
    }
    if (this.opts.app) {
    ...
    this.bootstrap = new Bootstrap(this.sdb, this.bootstrapPort, this.opts.websocket);
    await this.bootstrap.start(this.opts.appPackage);
    await this.bootstrap.pushUiAutomator();
  ...
  async deleteSession () {
    ...
    if (this.bootstrap) {
      ...
      await this.sdb.startAutoSleep();
      await this.bootstrap.removeUiAutomator();
