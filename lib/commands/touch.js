...
commands.scrollElementIntoView = async function (elementId) {
  ...
  if (needToScrollY) {
    ...
    /* BUG/TODO:
     * Some applications have non-scrollable bars at the top. These bars may be arbitrarily
     * high and if startY happens to be on such a bar the app will not scroll up!
     * Setting startY to screenResolution / 4.5 is reasonable and should work for most apps
     * but to be sure that this will always work, fix it (maybe check if an element under startX is scrollable?).
     */
    const startY = flickFromTopToBottom ? Math.round(this.screenResolution.y / 4.5) : (this.screenResolution.y - 1);
