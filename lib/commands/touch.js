...
commands.touchLongClick = async function (elementId) {
  ...
  return new Promise((resolve, reject) => {
    const call = this.aurumClient.longClick(clickReq,
      (error, result) => {
        throwOnUnsuccessfulAurumCall('longClick', error, result);
        return resolve();
      });
  });
};
...
commands.performTouch = async function () {
  // TODO: if touch{up, down, move} receives elementId as its
  // argument - first get coords of the element and then
  // use it to perform the action
